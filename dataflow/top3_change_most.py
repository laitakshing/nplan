...
def run(input_topic="projects/tak-playground/topics/one_min", schema='time:TIMESTAMP, top3:STRING', window_size=60, pipeline_args=None):
    ...
    5. Group the stock tuple by key with the the maximum difference of price change
    6. Use TopDistinctFn to get the top 3 highest price change of stocks
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        (
            ...
            | "Parse PubSub Messages to tuple" >> beam.Map(lambda elem: [(k, v) for k, v in elem.items() if k not in ["TIME"]])
            | 'Add Window info' >> beam.WindowInto(beam.window.SlidingWindows(window_size, 10))
            | "split all stock" >> beam.FlatMap(lambda e: e)
            | 'take the difference' >> beam.CombinePerKey(lambda elem: max(elem)-min(elem))
            | 'Top 3 stocks largest difference' >> beam.CombineGlobally(
            ...
            | 'WriteToBigQuery' >> beam.io.WriteToBigQuery(f'tak-playground:testing.table_difference_{str(int(window_size/60))}min', schema=schema,    write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND, create_disposition=beam.io.BigQueryDisposition.CREATE_IF_NEEDED)
        )
